...
- 관련 데이터와 `기능의 캡슐화`로 인한 코드 구조화
- 상속을 통한 `코드 재사용`과 `확장성`

단점

- this 바인딩으로 인한 복잡성 증가 가능성
...
- 순수 함수를 통한 예측 가능한 결과와 부작용 최소화
- 함수 합성을 통한 복잡한 동작의 단순화

단점

- 상태 관리가 필요한 복잡한 애플리케이션에서는 구현이 까다로울 수 있음
...
이 문제를 해결하기 위해, 우리는 클래스형 프라이빗 필드 문법을 사용하여 내부 로직을 은닉화하기로 결정했습니다. <br> 이 방법을 통해 객체 지향의 일관성을 유지하면서도, 필요한 부분만 외부에 노출시킬 수 있었습니다. <br><br>
...
이 새로운 접근 방식을 통해 우리는 코드 중복을 크게 줄일 수 있었고, 새로운 메시지 타입을 쉽게 추가할 수 있는 확장성을 확보했습니다. 모든 메시지가 동일한 기본 구조를 가지게 되어 데이터베이스 구조도 단순화되었으며, 단일 리스너 사용으로 리소스 사용 효율성도 개선되었습니다.

이러한 설계를 통해 우리는 다양한 DOM 조작 메서드를 효율적으로 처리하고, 실시간으로 다른 사용자와 공유할 수 있는 시스템을 구축할 수 있었습니다.
